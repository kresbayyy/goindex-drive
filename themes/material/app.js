...
function nav(path) {
    ...
    <a href="https://discord.gg/2YuZ2u5" target="_blank" class="mdui-btn mdui-btn-icon mdui-ripple mdui-ripple-white" mdui-tooltip="{content: 'Fumei on Discord'}">
      <svg id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 36 36" enable-background="new 0 0 36 36" xml:space="preserve" class="mdui-icon" style="width: 24px;height:24px;"><style>.st0{fill:#FFFFFF;}</style>
...
function list_files(path,files){
    ...
    for(i in files){
        ...
        if(item['mimeType'] == 'application/vnd.google-apps.folder'){
            ...
            var ext = p.split('.').pop();
            if("|js|json|txt|sh|md|".indexOf(`|${ext}|`) >= 0){
	            p += "?a=view";
	            c += " view";
            }
